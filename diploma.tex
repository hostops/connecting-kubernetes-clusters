...
Problem prevelike latence se pojavi v primeru počasnega prenosa podatkov iz naše gruče do uporabnikov~\cite{minimizing-latency}.
...
Zavedati se moramo, da strežniki v gruči zelo veliko komunicirajo, zato je priporočljivo, da so tudi v istem omrežju znotraj istega podatkovnega centra, saj se s tem izognemo veliki latenci ~\cite{kube-and-edge}.
\begin{figure}[h]
\begin{center}
...
Zaradi prevelike latence ali drugih ovir računalnikov ne moremo povezati v eno tesno gručo~\cite{kube-and-edge}.
...
Ob prisotnosti povezave pa lahko te gruče med seboj povežemo šibkeje.
...
\subsection{Service}
Objekt service označuje vse pode ene mikrostoritve~\cite{service}.
Kubernetes iz objekta v notranjem DNS ustvari domeno za mikrostoritev in dinamično razvršča promet med našimi podi.
Objekte service uporabljamo tako, da namesto pošiljanja zahtevkov direktno na IP naslov Poda, delamo klice na ustvarjeno domensko ime storitve na primer \spverb|curl ime-storitve|.
...
Objekt zelo podoben deploymentu, le da statefulset vsakemu podu dodeli unikatno številko~\cite{statefulset}. 
