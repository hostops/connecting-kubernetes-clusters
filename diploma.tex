...
Takšen pristop se je v zadnjih letih zelo razširil in ArgoCD navaja več kot 100 podjetji, ki pri razvoju svojih spletnih aplikacij uporablja pristop GitOps~\cite{argocd-user-list}.
...
Drugi način, ki pa ga ponuja ArgoCD pa je, da lahko konfiguracijo pošljemo tudi v oddaljene gruče~\cite{declarative-setup}. To pomeni, da imamo lahko kontroler ArgoCD nameščen samo v eni gruči.
...
Zaradi naprednih možnosti, ki jih Cilium ponuja, se podjetja velikokrat odločijo za uporabo Ciliuma v svojih gručah Kubernetes, tudi ko ne povezujejo več gruč med seboj.
...
K sreči pa CrateDB implementira vmesnik PostgreSQL in nam kode za prehod med bazami ni potrebno spreminjati.
...
\label{primer-uporabe-helm-predloge-argo-cd}
\end{figure}
Vsa konfiguracija je javno objavljena na repozitoriju Git\cite{git-diploma}. Postavimo jo z ukazom \spverb|kubectl apply -f diploma-demo-1|
