...
Zavedati se moramo, da strežniki v gruči zelo veliko komunicirajo zato je priporočljivo, da tudi fizično blizu skupaj, da se izognemo latenci. 
...
V splošnem se problem reši tako, da se naše gruče namestimo v več podatkovnih centrov.
...
Glede izolacije spletnih aplikacij smo z uporabo Kubernetesa naredili že kar nekaj korakov k rešitvi problema.
...
Torej ima zaradi tega, ker ima administratorske pravice na vozlišču, posledično tudi popolno kontrolo nad vsemi drugimi aplikacijami, ki tečejo na tem vozlišču.
...
V takšnih primerih po navadi lahko vsaj računalnike na isti lokaciji povežemo v gručo.
...
Odvisno od naših potreb pa si lahko želimo tudi, da nam sistem omogoča dinamično odkrivanje servicov z enako definicijo v različnih gručah, komunikacijo med servici v različnih gručah, dinamično odkrivanje podov med gručami in komunikacijo med podi v različnih gručah.
...
Z uporabo ukaza kubefedctl join povežemo vse tri gruče v kubefed sistem.
\begin{verbatim}
...
Če je čim višja razpoložljivost za našo aplikacijo kritičnega pomena in smo že poskrbeli za visoko razpoložljivost (HA) aplikacije v naši gruči, še vedo lahko pride do situacije, ko iz omrežja izpade cel podatkovni center.
...
Če nam ni dovolj niti, da uporabimo različne razpoložljivostne cone in podatkovne centre oblačnih ponudnikov, lahko postavimo naše gruče pri več različnih ponudnikih.
...
Postaviti moramo gruče v več različnih podatkovnih centrov in jih nastaviti, da bodo delovali sinhrono.
...
Ko imamo na vsaki gruči izpostavljen service in urejene ingress zapise moramo še vedno uporabnike preusmeriti na njim najbližjo gručo.
...
V našem primeru spletne aplikacije bomo imeli v vsaki gruči en deployment z našo aplikacijo Stateful rest sample in en pripadajoči service, da aplikacijo izpostavimo izven gruče.
Za podatkovno bazo pa bomo uporabili statefulset s CrateDB, diskom na lokalni SD kartici, internim servicom za prepoznavo ostalih instanc CrateDB v gruči in eksterni service, ki ga uporabimo za povezovanje na podatkovno bazo.
\begin{figure}[h]
\begin{center}
...
Po preizkusu delovanje s curl ukaz opazimo, da podatki med gručami še vedno niso sinhronizirani.
...
Sicer v našem primeru res uporabljamo samo eno instanco CrateDB baze na gručo a vseeno smo na nivoju sinhronizacije znotraj gruče to stvar že uredili. 
...
Vseeno pa kljub temu, da so vdori in napake, zaradi katerih postanejo nedosegljive naše gruče vedno lahko poizkusimo zmanjšati posledice ob morebitnem vdoru ali napadu. 
\subsection{Izpadi aplikacije}
Kljub temu, da smo naše aplikacije namestili na različne gruče in je s tem aplikacija odporna na izpad ene gruče ga ob hujših nepravilnostih delovanja aplikacije in nastavitev gruče lahko zaradi napake na eni aplikaciji izpadejo vse gruče na katerih imamo aplikacijo nameščeno.
...
Najprej bomo z ukazom kubectl tag označili naš izolirano gručo (ali več njih).
Potem pa bomo lastnost spec.placement.clusterSelector.matchLabels vsakega federiranega objekta, ki ga želimo izolirati dodali označbo vseh izoliranih gruč.
...
\section{Sinhronizacija podatkov}
V primeru gruč na robu oblaka bomo sinhronizacijo verjetno implementirali na novo, saj le mi vemo kakšen problem rešujemo in zakaj smo sploh postavljali gruče na robu oblaka.
